...
public class Bat extends Mammal implements Flying {
	...
	// using 9 parameters may be too much. Would like to use builder pattern
	// later on to condense it down later on. Don't know how to use this.

	public Bat(int age, String name, String species, String gender, int weight, int height, int wastePerDay,
			int breathsPerMin, int foodQuantity, int populationNumber, int averageLifeSpan, boolean endangered,
			boolean nocturnal, int flightSpeed, int power, int landingSpeed) {
	...
	public void fly() { // lighter the bat can increase its flight speed
		if (getWeight() < 3) {
			...
			this.setFlightSpeed(getFlightSpeed() - 2); // heavier bat the flight
														// speed goes down
